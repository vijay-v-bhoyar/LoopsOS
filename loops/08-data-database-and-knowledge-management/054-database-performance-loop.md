# Database Performance Loop

## Loop Identity

- Loop number: 54
- Loop name: Database Performance Loop
- Category: 8. Data, Database and Knowledge Management
- Skill ID: `loop-054-database-performance-loop`
- Version: `1.0`
...
Outcome to improve: Database performance results and approved tuning changes.

Primary measurable outcome: Database performance results and approved tuning changes.

Business value: Improves confidence, speed, safety, and accountability for Database Performance Loop decisions by tying every action to evidence and proof.

Risk reduced: Reduces the risk that Database Performance Loop produces unowned gaps, stale evidence, unproven claims, unsafe actions, repeated failures, or ineffective improvements.

Scope: Data, Database and Knowledge Management control point for Database Performance Loop.
...
Systems and data covered: Systems, workflows, data, models, agents, controls, evidence, and decisions directly involved in `Database Performance Loop`.
...
Slow query, capacity alert, lock contention, replication lag, or scheduled review.
...
- Trigger condition: Slow query, capacity alert, lock contention, replication lag, or scheduled review.
- Priority: To be calculated from impact, urgency, risk, and confidence
- Deduplication key: `loop-054-database-performance-loop:<asset-or-scope>:<trigger-signature>`
...
Inspect execution plans, indexes, partitions, locks, connection use, storage, replication, and configuration; tune and retest under realistic load.

## Output

Database performance results and approved tuning changes.
...
- Loop record for `loop-054-database-performance-loop`
...
- Current state metric: To be measured for `Database Performance Loop`
...
- Metric: Primary measurable outcome for `Database Performance Loop`
...
```yaml
loop_id: "loop-054-database-performance-loop"
name: "Database Performance Loop"
version: "1.0"
...
- Loop: Database Performance Loop
- Outcome: Database performance results and approved tuning changes.
- Trigger: Slow query, capacity alert, lock contention, replication lag, or scheduled review.
- Qualified:
- Owner:
- Evidence observed:
- Diagnosis:
