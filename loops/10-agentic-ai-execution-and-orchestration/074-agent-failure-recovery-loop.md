...
- Loop number: 74
- Loop name: Agent Failure Recovery Loop
- Category: 10. Agentic AI Execution and Orchestration
- Skill ID: `loop-074-agent-failure-recovery-loop`
- Version: `1.0`
...
- Minimum cadence: Every agent failure
...
Outcome to improve: Recovered workflow or controlled failure with diagnostic evidence.

Primary measurable outcome: Recovered workflow or controlled failure with diagnostic evidence.

Business value: Improves confidence, speed, safety, and accountability for Agent Failure Recovery Loop decisions by tying every action to evidence and proof.

Risk reduced: Reduces the risk that Agent Failure Recovery Loop produces unowned gaps, stale evidence, unproven claims, unsafe actions, repeated failures, or ineffective improvements.

Scope: Agentic AI Execution and Orchestration control point for Agent Failure Recovery Loop.
...
Systems and data covered: Systems, workflows, data, models, agents, controls, evidence, and decisions directly involved in `Agent Failure Recovery Loop`.
...
Planning, model, tool, data, permission, policy, or execution failure.
...
- Trigger condition: Planning, model, tool, data, permission, policy, or execution failure.
- Priority: To be calculated from impact, urgency, risk, and confidence
- Deduplication key: `loop-074-agent-failure-recovery-loop:<asset-or-scope>:<trigger-signature>`
...
Classify the failure, determine whether retry is safe, apply limits and backoff, select an alternate path, roll back partial work, or escalate to a human.

## Output

Recovered workflow or controlled failure with diagnostic evidence.
...
- Loop record for `loop-074-agent-failure-recovery-loop`
...
- Current state metric: To be measured for `Agent Failure Recovery Loop`
...
- Metric: Primary measurable outcome for `Agent Failure Recovery Loop`
...
Minimum frequency: Every agent failure
...
```yaml
loop_id: "loop-074-agent-failure-recovery-loop"
name: "Agent Failure Recovery Loop"
version: "1.0"
...
- Loop: Agent Failure Recovery Loop
- Outcome: Recovered workflow or controlled failure with diagnostic evidence.
- Trigger: Planning, model, tool, data, permission, policy, or execution failure.
- Qualified:
- Owner:
- Evidence observed:
- Diagnosis:
