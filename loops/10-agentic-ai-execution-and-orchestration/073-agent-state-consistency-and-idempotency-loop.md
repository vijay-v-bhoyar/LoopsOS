...
- Loop number: 73
- Loop name: Agent State Consistency and Idempotency Loop
- Category: 10. Agentic AI Execution and Orchestration
- Skill ID: `loop-073-agent-state-consistency-and-idempotency-loop`
- Version: `1.0`
...
- Minimum cadence: Every state-changing action
...
Outcome to improve: Consistent system state and state-transition evidence.

Primary measurable outcome: Consistent system state and state-transition evidence.

Business value: Improves confidence, speed, safety, and accountability for Agent State Consistency and Idempotency Loop decisions by tying every action to evidence and proof.

Risk reduced: Reduces the risk that Agent State Consistency and Idempotency Loop produces unowned gaps, stale evidence, unproven claims, unsafe actions, repeated failures, or ineffective improvements.

Scope: Agentic AI Execution and Orchestration control point for Agent State Consistency and Idempotency Loop.
...
Systems and data covered: Systems, workflows, data, models, agents, controls, evidence, and decisions directly involved in `Agent State Consistency and Idempotency Loop`.
...
State-changing action, retry, timeout, parallel execution, or recovery.
...
- Trigger condition: State-changing action, retry, timeout, parallel execution, or recovery.
- Priority: To be calculated from impact, urgency, risk, and confidence
- Deduplication key: `loop-073-agent-state-consistency-and-idempotency-loop:<asset-or-scope>:<trigger-signature>`
...
Use operation identifiers, checkpoints, transactions, locks where appropriate, and idempotency controls; detect duplicates, conflicts, replay, and partial completion; reconcile or compensate.

## Output

Consistent system state and state-transition evidence.
...
- Loop record for `loop-073-agent-state-consistency-and-idempotency-loop`
...
- Current state metric: To be measured for `Agent State Consistency and Idempotency Loop`
...
- Metric: Primary measurable outcome for `Agent State Consistency and Idempotency Loop`
...
Minimum frequency: Every state-changing action
...
```yaml
loop_id: "loop-073-agent-state-consistency-and-idempotency-loop"
name: "Agent State Consistency and Idempotency Loop"
version: "1.0"
...
- Loop: Agent State Consistency and Idempotency Loop
- Outcome: Consistent system state and state-transition evidence.
- Trigger: State-changing action, retry, timeout, parallel execution, or recovery.
- Qualified:
- Owner:
- Evidence observed:
- Diagnosis:
