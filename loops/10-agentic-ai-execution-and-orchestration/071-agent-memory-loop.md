...
- Loop number: 71
- Loop name: Agent Memory Loop
- Category: 10. Agentic AI Execution and Orchestration
- Skill ID: `loop-071-agent-memory-loop`
- Version: `1.0`
...
- Minimum cadence: Every memory operation, with scheduled cleanup
...
Outcome to improve: Accurate, relevant, and governed memory state.

Primary measurable outcome: Accurate, relevant, and governed memory state.

Business value: Improves confidence, speed, safety, and accountability for Agent Memory Loop decisions by tying every action to evidence and proof.

Risk reduced: Reduces the risk that Agent Memory Loop produces unowned gaps, stale evidence, unproven claims, unsafe actions, repeated failures, or ineffective improvements.

Scope: Agentic AI Execution and Orchestration control point for Agent Memory Loop.
...
Systems and data covered: Systems, workflows, data, models, agents, controls, evidence, and decisions directly involved in `Agent Memory Loop`.
...
Memory read, write, update, expiry, correction, or deletion request.
...
- Trigger condition: Memory read, write, update, expiry, correction, or deletion request.
- Priority: To be calculated from impact, urgency, risk, and confidence
- Deduplication key: `loop-071-agent-memory-loop:<asset-or-scope>:<trigger-signature>`
...
Decide whether information should be stored, classify sensitivity, obtain required consent, deduplicate, resolve conflicts, set retention, retrieve selectively, and support correction and deletion.

## Output

Accurate, relevant, and governed memory state.
...
- Loop record for `loop-071-agent-memory-loop`
...
- Current state metric: To be measured for `Agent Memory Loop`
...
- Metric: Primary measurable outcome for `Agent Memory Loop`
...
Minimum frequency: Every memory operation, with scheduled cleanup
...
```yaml
loop_id: "loop-071-agent-memory-loop"
...
- Loop: Agent Memory Loop
- Outcome: Accurate, relevant, and governed memory state.
- Trigger: Memory read, write, update, expiry, correction, or deletion request.
- Qualified:
- Owner:
- Evidence observed:
- Diagnosis:
