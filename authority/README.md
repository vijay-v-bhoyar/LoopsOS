...
The release-assurance UI uses `POST /v1/release-initiatives/record` for session-authenticated evidence. Its request contains the initiative draft and connector evidence inputs; the authority records every evidence event and the initiative in one database transaction, derives the initiative's `source_event_ids` from the committed event records, and returns both the durable initiative and event records. A validation, authorization, idempotency, or persistence failure rolls the whole unit back, so a retry cannot leave orphaned evidence without a release record. Provider-verified events remain restricted to signed webhook ingestion.
...
The authority also records a deterministic readiness verdict for each release initiative. `GO` requires fresh connector evidence, every linked event to be `verified_webhook`, and all release assurance gates passed. Session-authenticated snapshots are useful shadow evidence but produce `REVIEW_REQUIRED` until an upstream-verified event is recorded. Stale or missing evidence, missing gates, blocked gates, evidence gaps, and unrecognized gate statuses produce `NO_GO`. Review-required gates and active exceptions produce `REVIEW_REQUIRED` only when no fail-closed condition is present. This verdict is a readiness decision for the recorded release packet; production deployment still requires a separate governed action with its own payload-bound approval and rollback contract.
...
These records do not claim live Jira or GitHub authority by themselves. Connector reads and write-back remain separate governed actions: shadow-read evidence can support a proof pack, while comments, status checks, issue updates, or release decisions must go through payload-bound approval and the existing tool/action contract. Outbound governed HTTP actions include the durable `X-LoopOS-Run-ID` and `X-LoopOS-Invocation-ID` correlation headers; retries preserve both values and the action idempotency key so connector logs can be reconciled to the authority audit trail without receiving credentials.

Production connector credentials are fail-closed until an approved short-lived credential-injection broker is integrated. `LOOPOS_CONNECTOR_BEARER_TOKENS_JSON` is local-development-only; production readiness rejects it and runtime dispatch refuses to send it, rather than treating a static secret as brokered authority. `/health/ready` reports `credential_injection_broker_verified=false` until that real broker integration exists, and the enterprise UI must not start while that proof is false.
...
| `LOOPOS_RATE_LIMIT_REQUESTS` | Positive request count allowed per client during one fixed window; required outside development |
| `LOOPOS_RATE_LIMIT_WINDOW_SECONDS` | Positive fixed-window duration in seconds; required outside development |
...
| `LOOPOS_WEBHOOK_SECRETS_JSON` | Server-only JSON map of `tenant:system` to a minimum-32-byte HMAC secret for enterprise verified webhook evidence; local development may use a shared `system` key |
...
| `LOOPOS_AUDIT_ANCHOR_DELIVERY_MAX_AGE_SECONDS` | Maximum age of the most recent verified anchor delivery for readiness; defaults to 300 seconds |
...
| `LOOPOS_EXECUTION_WORKER_MODE` | `internal` for a long-lived worker loop or `external` for protected cron dispatch; defaults to `external` on Vercel, where `internal` is rejected |
...
The static production gate remains `NO_GO` until an approved credential-injection broker is independently integrated and verified. Absence of static connector tokens is necessary but does not prove broker integration.

Development sessions deliberately make local evaluation easy. Enterprise deployment must disable them and place an identity-aware gateway in front of the service. The gateway supplies a signed OIDC assertion through the same-origin session exchange; LoopOS verifies the fixed issuer, audience, RS256 signature, expiry, tenant claim, and explicit external-role mapping before issuing a 15-minute application session. Session issuance creates an audited tenant event and immediately attempts delivery through the signed external-anchor outbox. Production readiness rejects missing or malformed request-rate limits, missing identity configuration, SQLite persistence, missing audit-anchor configuration, an audit sink that has never accepted a signed envelope, undelivered anchor backlog, missing durable-worker dispatch, missing retention/support/outbound bindings, and missing or stale restore-evidence references.

Starting, rolling back, and delayed-effectiveness work is committed to `execution_jobs` in the same database transaction as the run status. Workers claim rows with renewable leases; Postgres uses row locks and `SKIP LOCKED`; expired work can be reclaimed after process death. Every successful poll records a durable, mode-specific heartbeat, and production readiness rejects a missing, stale, or wrong-mode heartbeat. Long-lived deployments poll internally. Serverless deployments require an external scheduler to invoke `POST /api/v1/operations/jobs/drain` with `X-LoopOS-Worker-Token` or `Authorization: Bearer <CRON_SECRET>`. Schedule it more frequently than `LOOPOS_EXECUTION_WORKER_HEARTBEAT_MAX_AGE_SECONDS` and verify the durable heartbeat before handover. The handover verifier also creates a bounded R1 run through the public API and requires the external drain to return it as `EFFECTIVENESS_PROVEN` with `runner_status=completed`; a heartbeat alone cannot establish worker readiness. The schedule is deliberately not embedded in `vercel.json`, because sub-daily Vercel cron support is plan-specific; Vercel Pro cron or an independently managed scheduler may provide the binding. Each dispatch also drains the audit-anchor outbox. The sink and tool contracts must remain idempotent because a lease-recovery path can repeat a request after an ambiguous process failure.

Every audit-event insert atomically creates an outbox envelope containing the chain hashes and canonical event fields. The worker signs the exact canonical request bytes with `x-loopos-signature-256`, supplies `x-loopos-event-id` for sink-side idempotency, marks only 2xx responses delivered, and retains bounded failure details with exponential retry timing. Auditors and executives can inspect `GET /v1/audit/anchors/status`; executives can request an immediate retry for their own tenant through `POST /v1/audit/anchors/drain`. Platform-wide delivery remains worker-token-only.

Authenticated session users may record shadow connector snapshots through `POST /v1/connector-events`, but that route cannot claim provider verification or attach a delivery identifier. `verified_webhook` evidence is created only by the signed provider webhook routes after HMAC and provider-delivery validation.
...
Before deploying, validate the server-side environment without starting the API or printing secret values:

```powershell
python scripts/verify_production_environment.py --output output/production-configuration.json
```

`READY_FOR_LIVE_VERIFICATION` means only that the static production configuration is complete. The gate also requires the public enterprise UI posture to match the authority's retention, support, outbound, and restore bindings, and rejects optional service endpoints outside the server allowlist. It never proves deployment routing, database connectivity, identity assertions, tenant isolation, audit delivery, worker heartbeat, or authoritative handover; those remain mandatory live gates.
